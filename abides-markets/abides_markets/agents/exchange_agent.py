...
class ExchangeAgent(FinancialAgent):
    ...
    def kernel_terminating(self) -> None:
        ...
        # Aggregate exchanged volume across *all* symbols this exchange manages
        total_exchanged = 0
        lookback = self.current_time - self.mkt_open
        for sym in self.symbols:
            bid_volume, ask_volume = self.order_books[sym].get_transacted_volume(lookback)
            total_exchanged += (bid_volume + ask_volume)
        self.total_exchanged_volume = total_exchanged
